#' heat_risk_index_value
#'
#' Calculate the heat risk value for worker by using color codes returning index value.
#'
...
heat_risk_index_value=function(wbgt,cav,tresh)  {
  ct$assign("wbgt", as.array(wbgt))
  ct$assign("cav", as.array(cav))
  ct$assign("tresh", as.array(tresh))
  ct$eval("var res=[]; for(var i=0, len=wbgt.length; i < len; i++){ res[i]=heat_risk_index_value(wbgt[i],cav[0],tresh[0])};")
  res=ct$get("res")
  return(res)
}
